...
const tagUrl = (url) => {
  ...
  try {
    const u = new URL(url);
    if (AFFILIATE.amazon && (u.hostname.includes("amazon.com") || u.hostname.includes("amzn.to"))) {
...
const loadData = () => { try { const r = localStorage.getItem(STORAGE_KEY); if (r) return migrate(JSON.parse(r)); const old = localStorage.getItem("homesked-data-v2"); if (old) return migrate(JSON.parse(old)); } catch(e) { console.error(e); } return []; };
  ...
  const PartLine = ({p, muted}) => (
    <div style={S.partRow}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        {p.purchaseUrl && <a href={tagUrl(p.purchaseUrl)} target="_blank" rel="noopener noreferrer" style={S.buyLink} onClick={e=>e.stopPropagation()}>Buy</a>}
      </div>
    </div>
  );
  return (
  ...
  if (!onboarded) {
    ...
    return (
      <div className="app-container" style={S.app}>
        <div style={S.onboard}>
          <div style={S.onboardHero}><span style={{fontSize:56}}>🏡</span><h1 style={S.onboardTitle}>Home Sked</h1><p style={S.onboardSub}>Never miss a maintenance task again. Pick the systems in your home to get started.</p></div>
      ...
      <header style={S.header}>
        <div style={S.headerInner}>
          <div style={S.logoRow} onClick={()=>{setView("dashboard");setSelectedSystem(null);setListView(null);}}>
            ...
            <div><h1 style={S.logoTitle}>Home Sked</h1><p style={S.logoSub}>Maintenance Tracker</p></div>
          </div>
...
const S = {
  ...
  logoRow:{display:"flex",gap:12,alignItems:"center",cursor:"pointer"},logoIcon:{fontSize:28},
