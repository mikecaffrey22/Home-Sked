    ...
    return (
      <div className="app-container" style={{...S.app,background:K.bg,color:K.text}}>
        <div style={S.onboard}>
          ...
          <div style={{background:K.accentLight,border:"1.5px solid "+K.accent,borderRadius:K.radius,padding:"16px",marginTop:16,textAlign:"center"}}>
            <div style={{fontSize:13,fontWeight:700,fontFamily:sf,color:K.accent,marginBottom:4}}>📄 Have an appliance manual?</div>
            <p style={{fontSize:12,color:K.textMuted,fontFamily:sf,marginBottom:10,lineHeight:1.5}}>Upload a PDF or photo and AI will create a full maintenance schedule for you automatically.</p>
            <button style={{background:K.accent,color:"#fff",border:"none",borderRadius:8,padding:"10px 22px",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:sf,minHeight:40}} onClick={()=>{if(homes.length===0){const h={id:genId(),name:"My Home",icon:"🏡",systems:[]};setHomes([h]);setActiveHomeId(h.id);}setOnboarded(true);localStorage.setItem(ONBOARDED_KEY,"true");handleManualFileSelect();}}>Upload Manual</button>
          </div>
